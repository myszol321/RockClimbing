    ...
    async function setAll() {
      ...
      if (props.userInfo.id.toString() === profileId) {
        ...
        const response_user = await fetch(
          `http://localhost:4000/users/${profileId}`
        );
        const data_user = await response_user.json();
        setUserInfo(data_user[0]);
      }
      console.log(profileId);
      const response_events = await fetch(
        `http://localhost:4000/events/user/${profileId}`
      );
      const data_events = await response_events.json();
      const events = data_events.map((entry) => {
        return (
          <Event key={entry.id} {...entry} handleClick={props.handleClick} />
        );
      });
      setEventData(events);

      const response_stars = await fetch(
        `http://localhost:4000/users/${profileId}/stars`
      );
  ...
  const changeRating = (e) => {
    ...
    if (userInfo.id === props.userInfo.id) {
      ...
      const fetchedData = fetch(
        `http://localhost:4000/users/${userInfo.id}/stars`,
        {
          method: 'POST',
          mode: 'cors',
          body: JSON.stringify({
            my_id: props.userInfo.id,
  ...
  return (
    <div className='profile'>
      ...
      <div className='profile--events'>
        <h2>Wydarzenia</h2>
        <div className='profile-events'>{eventData}</div>
      </div>
    </div>
  );
}
