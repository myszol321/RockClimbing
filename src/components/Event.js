...
export default function Event(props) {
  ...
  const date =
    JSON.stringify(props.date_and_time).slice(1, 11) +
    ' ' +
    JSON.stringify(props.date_and_time).slice(12, 17);
  ...
  return (
    <div className='entry'>
      ...
      <div className='entry--info'>
        <p>
          Dodano przez: <b>{props.first_name}</b>
        </p>
      </div>
      <button className='entry--join'>
        <span onClick={openEvent}>Czytaj więcej</span>
      </button>
    </div>
  );
}
